...
| Location | Format | Role |
|----------|--------|------|
| `~/.local/share/opencode/opencode.db` | SQLite | Primary (~63 MB observed on-machine) |
| `~/.local/share/opencode/storage/session/global/ses_*.json` | JSON | Legacy (pre-SQLite format) |
