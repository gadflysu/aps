...
**Review and merge:** Open a PR with a title without `#N` or `(#N)` issue suffixes and put `Closes #N` in the body; wait for checks to pass; `cd` to the main worktree, run `gh pr merge N --squash -d`, delete the local issue branch, pull master, confirm local/remote issue branches are gone, then evaluate CHANGELOG + patch tag if user-visible code shipped.
